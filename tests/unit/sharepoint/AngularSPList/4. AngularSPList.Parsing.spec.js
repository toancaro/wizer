(function () {
    ...
    describe("AngularSPList", function () {
        suites = ["[deprecated] parsing"];
        describes(suites, function () {
            ...
            describe("request", function () {
                xit("should use: fields configs -> converters -> schema", function () {
                    expect(parsedClientItem.orderField).toEqual(null);
                });

                describe("[deprecated] using field's `beforePost`", function () {
                ...
                describe("using schema", function () {
                    it("[deprecated] should use `beforePost`", function () {
                        expect(typeof (parsedClientItem.redundantField)).toEqual("undefined");
                    });
                    it("[deprecated] should use `beforePost` as promise", function () {
                        list = list.define({
                            schema: {
                                beforePost: function (clientItem) {
                                    return $q.when().then(function () {
                            ...
                            fields: {
                                // overrite this to prevent parsing stupid room name.
                                roomName: {
        ...
        describes(suites, function () {
            ...
            function testSchema(schemaType, parseType) {
                describe(schemaType, function () {
                    ...
                    beforeEach(function () {
                        var configs = {};
                        _.set(configs, "schema." + schemaType + "." + parseType, []);
                    ...
                    function push(fnArray) {
                        var configs = list.configs();
                        var pipeline = _.get(configs, String.format("schema.{0}.{1}", schemaType, parseType));
            ...
            /**
             * Test order of invoking parsing functions.
        ...
        suites = ["parsing", "response"];
        describes(suites, function () {
            var $rootScope, $SPList, baseList;
            ...
            describe("type", function () {
                it("json", function () {
                    var result;
                    var data = {json: '{"name": "Jubei"}'};

                    var list = baseList.define({
                        fields: {
                            json: {type: "json"}
                        }
                    });

                    list.$$parseServerItem(data).then(function (parsed) {
                        result = parsed;
                    });

                    $rootScope.$digest();
                    expect(result.json).toEqual({name: "Jubei"});
                });
                it("datetime", function () {
                    var result, datetime = new Date();
                    var data = {datetime: datetime.toJSON()};
                ...
                it("lookup + person", function () {
                    var result, lookup = {Id: 1};
                    var data = {lookup: lookup, person: lookup};
                    ...
                    $rootScope.$digest();
                    expect(result.lookup).toEqual(lookup);
                    expect(result.person).toEqual(lookup);
                });
                it("multi-lookup", function () {
                    var result;

                    var lookupData = [{Id: 1}, {Id: 2}];
                    var lookup = {results: lookupData};
                    var data = {multilookup: lookup, people: lookup};

                    var list = baseList.define({
                        fields: {
                            multilookup: {type: "multilookup"},
                            people: {type: "people"}
                        }
                    });
                    ...
                    $rootScope.$digest();
                    expect(result.multilookup).toEqual(lookupData);
                    expect(result.people).toEqual(lookupData);
                });
            });
        });

        suites = ["parsing", "request"];
        describes(suites, function () {
            var $rootScope, $SPList, baseList;
            ...
            describe("type", function () {
                it("json", function () {
                    ...
                    list.$$parseClientItem(data).then(function (parsed) {
                        result = parsed;
                    });

                    $rootScope.$digest();
                    expect(result.json).toEqual(JSON.stringify({name: "Jubei"}));
                });
                it("datetime", function () {
                    var result, datetime = new Date();
                    ...
                    $rootScope.$digest();
                    expect(result.datetime).toEqual(datetime.toJSON());
                });
                it("lookup + person", function () {
                    var result, lookup = {Id: 1};
                    var data = {lookup: lookup, person: lookup};
                ...
                it("multi-lookup + people", function () {
                    var result, lookup = [{Id: 1}, {Id: 2}];
                    var data = {multilookup: lookup, people: lookup};
                    ...
                    $rootScope.$digest();
                    expect(result).toEqual({
                        multilookupId: {results: _.pluck(lookup, "Id")},
                        peopleId: {results: _.pluck(lookup, "Id")}
                    });
                });
            });
            describe("readonly", function () {
                it("should remove readonly field from request object", function () {
                    var result;
                    var data = {readonly: "do not post this"};
